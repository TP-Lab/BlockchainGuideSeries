...
> 以太坊的出块速度为15秒，不考虑回滚问题，一个交易要被记录在区块链上，大概要15秒的时间。
...
2018年区块链开始进入3.0时期，针对区块链2.0速度慢、高昂矿工费等问题进行优化，实现区块链的高并发、高可拓展等性能，而其中最为代表的便是EOS。


> EOS的出块速度为0.5秒，不考虑回滚问题，一个交易被记录在区块链上，仅需0.5秒


除了基础的存储转账功能外，还能与链上合约进行即时交互；钱包不再是简单的资产管理工具，更是一个公链生态服务平台；与此同时，单链钱包已无法满足用户需求，**<font color=#2980fe>越来越多的钱包往多链方向发展</font>**。
...
> 所以，只要我们确定了某一树枝上的某一个位置，那这片叶子就是确定的。
...
而手机钱包和网⻚钱包大部分属于轻节点钱包，轻钱包依赖区块链网络中的其他全节点，比如钱包服务商会运行一个全节点，同步所有数据，然后根据不同的钱包地址将数据进行划分，这样用户便可以既快又方便的使用钱包。

中心化钱包，是服务商提供的一个中心化的账本。用户进行充值时，将对应的数字货币往服务商提供的地址充值，服务商再将所有的货币归集到自己的某个账号，同时在自己的账本上记录上用户的充值额度；此后，用户每进行一次转账操作，都是在服务商的账本上进行记录，而非记录在区块链上。当前在交易平台进行注册的账号都是中心化钱包。
...
+ 由于私钥不方便记忆，因此出现了助记词，助记词只是私钥的另一种展现形式。一般由12或24个英文单词组成，为了方便国内用户，也有提供汉字版本助记词。只要你记住这些单词，按照顺序在钱包中输入，就能恢复钱包并且进行任意操作。如果别人拿到了你的助记词，就相当于拿到了你的私钥，就可以对你的资产进行掌控。
...
+ 为了进一步增强安全性，大部分钱包会采取密码的方式对私钥做二次加密。每个钱包的加密方法和存储方式是不一样的。当你把私钥导入钱包后，你的每一次授权操作，其实背后都涉及钱包进行私钥解密，然后再进行交易签名等复杂的计算。
...
> + 助记词、私钥、keystore一旦泄露，资产将立刻被他人掌控，此时你需要立即转移资产到其他地址，原先的账户不再使用。
...
> 当前部分钱包已经开始支持聚合交易，如TokenPocket钱包预计在2019年12月上线聚合交易所。
